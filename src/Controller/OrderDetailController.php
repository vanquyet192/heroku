...
class OrderDetailController extends AbstractController
{
    ...
    public function new(Request $request, OrderDetailRepository $orderDetailRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $orderDetailRepository->save($orderDetail, true);
...
public function show($id, OrderDetailRepository $orderDetailRepository): Response
{
    // Lấy danh sách sản phẩm có ID đơn hàng tương ứng
    $orderItems = $orderDetailRepository->findBy(['orderid' => $id]);
